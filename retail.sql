...
--Who is the best customer?RFM ANALYSIS(Categorize the cuctomers based on recency, frequency and monetary)
...
--RFM analysis
--DROP TABLE IF EXISTS rfma, into rfma
SELECT *, 
CASE
...
CONCAT(cast(rfm_recency as string), cast(rfm_frequency as string), cast(rfm_monetary as string)) as rfm_string
FROM `retaildatplusage.alldataretail.rfm`
...
CREATE SCHEMA IF NOT EXISTS rfms;
SELECT c.*, CONCAT(rfm_recency + rfm_frequency + rfm_monetary) as rfm_cell
cast(rfm_recency as string)+ cast(rfm_frequency as string) + cast(rfm_monetary as string)rfm_cell_string
from rfm_calc c
